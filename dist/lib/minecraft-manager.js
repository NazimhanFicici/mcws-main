...
export class MinecraftManager {
    ...
    addClient(id, socket, address, port) {
        const client = {
            id,
            socket,
            address,
    ...
    removeClient(id, immediate = false) {
    ...
    updateClientActivity(id, displayName) {
        const client = this.clients.get(id);
        if (client) {
    ...
    sendCommand(command, targetClientId) {
    ...
    addActivity(clientId, type, data, playerName) {
        const activity = {
            id: uuidv4(),
            timestamp: new Date(),
            clientId,
    ...
    processMessage(clientId, message) {
        ...
        const displayName = message.body?.sender || message.body?.properties?.Sender || message.body?.player?.name;
        ...
        if (worldDataSnapshot) {
            ...
            this.onDataCallback?.('world_data_snapshot', {
            ...
            this.onDataCallback?.('minecraft_generic_data', {
                data: message,
                clientId,
                timestamp: new Date().toISOString()
            }, clientId);
    ...
    extractWorldDataSnapshot(message) {
        if (!message.body?.message)
            return null;
        try {
            const messageData = JSON.parse(message.body.message);
